...
User-installed skills may still live under `~/.codex/skills`. Setup-owned prompt files and native-agent TOML defaults are intentionally omitted in plugin mode unless explicitly installed.
...
| `prometheus-strict-metis` | `gpt-5.4-mini` | high | Prometheus Strict requirements interviewer and ambiguity mapper (frontier-orchestrator, frontier) |
| `prometheus-strict-momus` | `gpt-5.4-mini` | high | Prometheus Strict adversarial plan critic and risk challenger (frontier-orchestrator, frontier) |
| `prometheus-strict-oracle` | `gpt-5.4-mini` | high | Prometheus Strict implementation readiness verifier and handoff judge (frontier-orchestrator, standard) |
