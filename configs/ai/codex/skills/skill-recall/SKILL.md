...
successful sessions on all hosts into two tiers: this machine's per-repo `.agent/skills/`
...
~/repos/hart-agent-homes/tools/distiller/distill retrieve "<what you are trying to do>" \
    --mode active \
    --lib ~/repos/ollie_notes/.agent/skills \
...
- Each hit prints name, description, and score components. Then **read the skill file**
  for full steps/pitfalls/verification before acting (`.agent/skills/<name>.md`, or
  `.agent/skills-pending/` for staged, lower-evidence hits).
- Working outside ollie_notes: pass that repo's `.agent/skills` as `--lib` instead.
