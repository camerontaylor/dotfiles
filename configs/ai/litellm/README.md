...
ccfw                   # proxy → Fireworks fleet (kimi-k2.6, glm-5.3, etc.)
...
|---|---|---|
| `glm-5.3-bg` | `glm-5.3` on api.z.ai/api/paas/v4 | 200K context, ~50 t/s reasoning |
| `glm-5-bg` | `glm-5` | |
...
| `glm-5.3-fast` | `accounts/fireworks/models/glm-5p3` |
...
|---|---|---|---|
| `fleet-opus` | `kimi-k2.6` (Fireworks) | `ccfw` foreground | `glm-5.3-fast` → `minimax-m2.7` |
| `fleet-sonnet` | `glm-5.3-fast` (Fireworks) | `ccfw` sub-agents | `kimi-k2.6` → `minimax-m2.7` |
...
`glm-4.7-cerebras` (with `glm-` prefix matching the existing `glm-5.3-bg` /
`glm-5.3-fast` pattern) passes the classifier. Both entries point at the same
