...
# matters on macOS, and it is the strictest reading — then fall back to PATH. Dual-parseable: deploy.bash carries the
# same block.
if [ -x /bin/bash ]; then
...
if (( _bash_major < 3 )) || { (( _bash_major == 3 )) && (( _bash_minor < 2 )); }; then
    if [ -z "$_bash_probe" ]; then
        print "FATAL: no bash found — /bin/bash absent and none on PATH; this deploy needs >= 3.2" >&2
    else
        print "FATAL: $_bash_probe is version $_bash_major.$_bash_minor; this deploy needs >= 3.2" >&2
    fi
    exit 2
fi
