...
| AI/LLM tool config (skills, providers, gateways, routing) | **agents repo** — `~/.local/agents` → `configs/ai/<tool>/` (claude-code, codex, opencode, ccr-router, portkey, codexbar, paseo, agent-orchestrator). Never dotfiles. Installing the CLI itself stays here: mise / `.default-npm-packages` / `70_runtime_installs.zsh` — see [Agents sibling repo](#agents-sibling-repo) |
