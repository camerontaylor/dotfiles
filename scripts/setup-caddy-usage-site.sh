...
# (the dashboard UI only prompts when an endpoint answers 401). Since the
# collector moved to neptune (2026-09-10) the bearer also authenticates
# /usage and /cost UPSTREAM — the non-loopback bind token-gates them.
...
# DASH_SRC is a MIRROR: the canonical token lives on the collector host
# (neptune, ~/.local/state/codexbar/dashboard-token, generated by dotfiles
# 78_codexbar_serve.zsh) and is pushed here over ssh whenever regenerated —
# the dotfiles fragment prints the exact push command.
DASH_SRC="/home/ctaylor/.local/state/codexbar/dashboard-token"
[ -s "$DASH_SRC" ] || { echo "ERROR: $DASH_SRC missing — push it from the collector host (neptune); see scripts/deploy.d/78_codexbar_serve.zsh." >&2; exit 1; }
