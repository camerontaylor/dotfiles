...
write_caddyfile() {
  ...
  # docs/caddy-ingress.md names configs/caddy/Caddyfile the tracked source of
  # truth, installed to /etc/caddy/Caddyfile. Honour that when it exists: the
  # live file has been hand-extended since May 2026 (telemetry, mcp.ceres,
  # immich, usage.wedrifid) and the generated template below carries none of
  # those, so generating over a real host would silently delete four routes.
  # A missing route fails *quietly* -- it falls through to the portless
  # wildcard and serves a 404 from the wrong backend.
  local tracked="$SCRIPT_DIR/../configs/caddy/Caddyfile"
  if [[ -f "$tracked" ]]; then
    echo "Installing tracked $tracked -> $CADDYFILE_PATH"
    if [[ -f "$CADDYFILE_PATH" ]] && ! cmp -s "$tracked" "$CADDYFILE_PATH"; then
      sudo cp -p "$CADDYFILE_PATH" "$CADDYFILE_PATH.bak-$(date +%Y%m%d-%H%M%S)"
    fi
    sudo install -m 644 -o root -g root "$tracked" "$CADDYFILE_PATH"
    return 0
  fi

  # No tracked file: bootstrapping a fresh host. Emit the machine-route
  # template, which is intentionally minimal -- add host-specific sites to
  # configs/caddy/Caddyfile afterwards so they survive the next run.
  echo "No tracked Caddyfile; writing generated template for $DOMAIN..."
...
write_caddy_env() {
  ...
  [[ -n "$preserved" ]] && echo "  preserved $(printf '%s\n' "$preserved" | grep -c .) non-managed key(s) in $CADDY_ENV_PATH"
}
